(ns sentei.methods.social
  "social.cljc — 剪定 — Council as Pruner (post-hoc pruning governance) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from sentei's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "剪定 — Council as Pruner (post-hoc pruning governance) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over sentei's own domain (Council as PRUNER (剪定者), not censor. The organism grows from its root unstoppably; branches manifest on the append-only Datom log; sentei…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "sentei R0: live social posting is Council Lv6+ + operator + "
